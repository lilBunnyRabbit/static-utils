...
import { useBackgroundRemove, useBackgroundRemoveSettings } from "@/services/background-remove";
import { createLazyFileRoute } from "@tanstack/react-router";
import clsx from "clsx";
import { ArrowRight, Ellipsis, Loader2 } from "lucide-react";
import React from "react";

export const Route = createLazyFileRoute("/tools/background-remove/")({
  component: BackgroundRemoveRoute,
  errorComponent: ErrorComponent,
});

function BackgroundRemoveRoute(): React.ReactNode {
  const { showInfo } = useNavbar();
  const [imageFile, setImageFile] = React.useState<ImageFile>();

  const [settings, setSetting] = useBackgroundRemoveSettings();
  const { refs, isLoading, state, data } = useBackgroundRemove(imageFile, settings);
        ...
        <div className="p-4 flex flex-col gap-2 justify-between h-full">
          <div className="space-y-2">
            ...
            {settings.colors?.map((color, i) => {
              return (
                <Input
                  ...
                  value={color}
                  onChange={(e) => {
                    const clone = [...settings.colors!];
                    clone[i] = e.target.value;
                    setSetting("colors", clone);
                  }}
                />
              );
            })}
            ...
            <Button
              ...
              className="w-full"
              onClick={() => {
                setSetting("colors", [...settings.colors, "#ffffff"]);
              }}
            >
              Add Color
            </Button>
